class User {
  ...
  async getOne(pool, username) {
    return (await pool.query(`SELECT * FROM users WHERE username = $1::text`, [username])).rows[0];
  }
  ...
  async deleteOne(pool, username) {
    return (
      await pool.query(`DELETE FROM users WHERE username = $1::text RETURNING *;`, [username])
    ).rows[0];
  }
