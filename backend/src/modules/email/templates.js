...
function generateReminderHtml(reminder) {
  ...
  return `
    <center class="wrapper" style="${ReminderStyles.global + ReminderStyles.wrapper}">
            ...
            <tbody>
                <tr style="${ReminderStyles.global}">
                    <td class="td-title" style="${
                      ReminderStyles.global + ReminderStyles.td + ReminderStyles.td_title
                    }">
                          ...
                          ReminderStyles.reminder_table
                            ...
                            <tbody>
                                <tr class="date-row" style="${
                                  ReminderStyles.global + ReminderStyles.date_row
                                }">
                                ...
                                <tr class="reminder-title-row" style="${
                                  ReminderStyles.global + ReminderStyles.reminder_title_row
                                }">
                                    <td style="${ReminderStyles.global + ReminderStyles.td}">
                                        <h2 class="reminder-title" style="${
                                          ReminderStyles.global + ReminderStyles.reminder_title
                                        }">
                                            ${reminder.title}
                                        </h2>
                                    </td>
...
function generateSummaryHtml(logs, rangeString) {
  ...
  function changesTable(log) {
    ...
    for (const field of Object.keys(log.before ?? log.after)) {
      let rowHtml = [
        `<td style="${
          SummaryStyles.global + SummaryStyles.td + SummaryStyles.changes_table__td
        }">${field}:</td>`,
      ];
        ...
        rowHtml.push(
          `<td
                    ...
                    >
                        <span
                            class="diff green"
                            style="${SummaryStyles.global + SummaryStyles.diff.green}"
                        >
                            ${aftValue === "" || aftValue == null ? `<i>empty</i>` : aftValue}
                        </span>
                    </td>`,
        );
      } else if (after === null) {
        rowHtml.push(
          `<td
                    ...
                    >
                        <span
                            class="diff red"
                            style="${SummaryStyles.global + SummaryStyles.diff.red}"
                        >
                            ${bfValue === "" || bfValue == null ? `<i>empty</i>` : bfValue}
                        </span>
                    </td>`,
        );
      } else {
        ...
        rowHtml.push(
          `<td
                    ...
                    >
                        <span
                            class=${`diff ${bfColor ?? ""}`}
                            style="${SummaryStyles.global + (SummaryStyles.diff[bfColor] ?? "")}"
                        >
                            ${bfValue === "" || bfValue == null ? `<i>empty</i>` : bfValue}
                        </span>
                    </td>`,
        ...
        rowHtml.push(
          `<td
                    ...
                    >
                        <span
                            class=${`diff ${aftColor ?? ""}`}
                            style="${SummaryStyles.global + (SummaryStyles.diff[aftColor] ?? "")}"
                        >
                            ${aftValue === "" || aftValue == null ? `<i>empty</i>` : aftValue}
                        </span>
                    </td>`,
  ...
  return `
        ...
        >
            <table
                class="main"
                style="${SummaryStyles.global + SummaryStyles.table + SummaryStyles.main}"
            >
                <tbody style="${SummaryStyles.global}">
                    <tr style="${SummaryStyles.global}">
                        <td
                            class="title"
                            style="${SummaryStyles.global + SummaryStyles.td + SummaryStyles.title}"
                        >
                            <center>
                                <h1
                                    style="${SummaryStyles.global + SummaryStyles.title__h1}"
                                >
                                    HS-TMS Weekly Summary
                                </h1>
                                <h4
                                    style="${SummaryStyles.global + SummaryStyles.title__h4}"
                                >
                                    ${rangeString}
                                </h4>
                            </center>
                    ...
                    <tr style="${SummaryStyles.global}">
                        <td
                            class="statistics"
                            style="${
                              SummaryStyles.global + SummaryStyles.td + SummaryStyles.statistics
                            }"
                        >
                            ${`This week, ${filteredLogs.length} change${
                              filteredLogs.length > 1 ? "s" : ""
                            } to the database resources
                                ${filteredLogs.length > 1 ? "were" : "was"} recorded:`}
                            <ul
                                style="${SummaryStyles.global + SummaryStyles.ul}"
                            >
                                ${Object.entries(methodsCount)
                                  .map((entry) =>
                                    entry[1] > 0
                                      ? `<li
                                            style="${
                                              SummaryStyles.global + SummaryStyles.statistics__li
                                            }"
                                        >
                                            ${entry[0]}: ${entry[1]}
                                        </li>`
                            ...
                            >
                                <thead style="${SummaryStyles.global}">
                                    <tr style="${SummaryStyles.global}">
                                        ...
                                        <th
                                            style="${
                                              SummaryStyles.global + SummaryStyles.logs_table__th
                                            }"
                                        >
                                            Changes
                                        </th>
